...
public class GameScreen implements Screen {
    ...
    public GameScreen(MyGdxGame game, String levelName) {
        ...
        _tiledMapRenderer = new OrthogonalTiledMapRenderer(_tiledMap, 1f/PhysicsUtility.PIXELS_PER_METER);
    ...
    private boolean loadMap(String levelName) {
        _tiledMap = new TmxMapLoader().load("gameworld/levels/demo_level.tmx");
        ...
        MapLayer solidColliders = (MapLayer)_tiledMap.getLayers().get("SolidLayerColliders");
        ...
        // Create player
        MapLayer spawnPoints = (MapLayer)_tiledMap.getLayers().get("SpawnPoints");
        MapObject playerObject = spawnPoints.getObjects().get("Player");

        if(playerObject != null) {
            float playerX = PhysicsUtility.convertPixelsToMeters(Float.parseFloat(playerObject.getProperties().get("x").toString()));
            float playerY = PhysicsUtility.convertPixelsToMeters(Float.parseFloat(playerObject.getProperties().get("y").toString()));
    ...
    private void createSolidCollider(MapObject mapObject) {
        ...
        if(objectType.equals("QuadCollider")) {
        ...
        else if(objectType.equals("TriangleCollider")) {
            float x = PhysicsUtility.convertPixelsToMeters(Float.parseFloat(mapObject.getProperties().get("x").toString()));
            float y = PhysicsUtility.convertPixelsToMeters(Float.parseFloat(mapObject.getProperties().get("y").toString()));
